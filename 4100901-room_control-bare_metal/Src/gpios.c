#include "gpios.h"                                     // Pin PC13 (Button)
...
// Compatibility helper used by tim.c
void gpio_setup_pin(GPIO_Typedef_t * GPIO, uint8_t pin, uint8_t mode, uint8_t af)
{
    // Set mode (including AF)
    GPIO->MODER &= ~(3 << (pin*2));
    GPIO->MODER |= (mode << (pin*2));

    // If mode is AF, set AFRL/AFRH accordingly
    if (mode == GPIO_MODE_AF) {
        if (pin < 8) {
            GPIO->AFRL &= ~(0xFU << (pin * 4));
            GPIO->AFRL |= ((uint32_t)(af & 0x0F) << (pin * 4));
        } else {
            uint8_t p = pin - 8;
            GPIO->AFRH &= ~(0xFU << (p * 4));
            GPIO->AFRH |= ((uint32_t)(af & 0x0F) << (p * 4));
        }
    }
}
